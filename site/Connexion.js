...
/**
 * @class Api
 * @classdesc Classe permettant de gérer les connexions entrantes des clients API
 */
class Api {
	/**
	...
	/**
	 * @function run
	 * @description Lance la connexion entrante des clients API
	 * @returns {void}
	 */
	run() {
		...
		this.client.on("message", (message) => {
			...
			if (parsedMessage.amount_players) {
				...
				debug(
					"Reçu une réponse par rapport à la quantité de personnes connectées"
				);
			}
	...
	/**
	 * @function setplayers
	 * @description Envoie une requête pour récupérer le nombre de joueurs connectés
	 * @returns {void}
	 */
	setplayers() {
		request.method = "getplayersamount";
